...
router.post('/harvest/save',
  [
    ...
    check('harvest_quantity', 'Harvest Quantity value is not valid').not().isEmpty().trim().escape(),
    check('harvest_unitofmeasure', 'Harvest Unit of Measure value is not valid').not().isEmpty().trim().escape(),
...
router.post('/harvest/update', [
  ...
  check('harvest_quantity', 'Harvest Quantity value is not valid').not().isEmpty().trim().escape(),
  check('harvest_unitofmeasure', 'Harvest Unit of Measure value is not valid').not().isEmpty().trim().escape(),
  check('harvest_blockchainhashid', 'Blockchain Hash ID value is not valid').not().isEmpty().trim().escape(),
  check('harvest_blockchainhashdata', 'Blockchain Hash Data value is not valid').not().isEmpty().trim().escape(),
  check('supplierproduce', 'Supplier Produce value is not valid').not().isEmpty().trim().escape(),
  check('harvest_bool_added_to_blockchain', 'Added to Blockchain value is not valid').not().isEmpty().trim().escape(),
  check('harvest_added_to_blockchain_by', 'Harvest Added to Blockchain by value is not valid').not().isEmpty().trim().escape(),
  check('harvest_blockchain_uuid', 'Harvest Blockchain UUID value is not valid').not().isEmpty().trim().escape(),
  ...
  if (!result.isEmpty()) {
    ...
    let harvest_TimeStamp = moment(new Date(req.body.harvest_timestamp)).format("YYYY-MM-DD HH:mm:ss");
    let harvest_CaptureTime = moment(new Date(req.body.harvest_capturetime)).format("YYYY-MM-DD HH:mm:ss");
...
router.post('/harvest/save/whatsapp',
  async function (req, res) {
    let harvest_logid_uuid = uuidv4()
...
router.post('/storage/save', [
    ...
    check('storage_Description', 'Storage Description value is not valid').not().isEmpty().trim().escape(),
  ],
  function (req, res) {
    const result = validationResult(req);
    let errors = result.errors;
            ...
            models.FoodprintHarvest
              ...
              .then(harvest_rows => {
                ...
                let weeklyViewData = {
                  ...
                  storage_added_to_blockchain_by: sys_storage_added_to_blockchain_by,
                  storage_blockchain_uuid: sys_storage_blockchain_uuid,
                  harvest_bool_added_to_blockchain: harvest_rows[0].harvest_bool_added_to_blockchain,
...
router.post('/storage/update', [
    ...
    check('market_URL', 'Market URL value is not valid').not().isEmpty().trim().escape(),
    check('storage_BlockchainHashID', 'Blockchain Hash ID value is not valid').not().isEmpty().trim().escape(),
    check('storage_BlockchainHashData', 'Blockchain Hash Data value is not valid').not().isEmpty().trim().escape(),
    check('storage_Description', 'Storage Description value is not valid').not().isEmpty().trim().escape(),
    check('storage_bool_added_to_blockchain', 'Added to Blockchain value is not valid').not().isEmpty().trim().escape(),
    check('storage_added_to_blockchain_by', 'Storage Added to Blockchain by is not valid').not().isEmpty().trim().escape(),
    check('storage_blockchain_uuid', 'Storage Blockchain UUID value is not valid').not().isEmpty().trim().escape(),
  ...
  function (req, res) {
    ...
    if (!result.isEmpty()) {
      ...
      console.log('req.body.harvest_logid ' + req.body.harvest_logid);
      let storage_TimeStamp = moment(new Date(req.body.market_storageTimeStamp)).format("YYYY-MM-DD HH:mm:ss");
      let storage_CaptureTime = moment(new Date(req.body.market_storageCaptureTime)).format("YYYY-MM-DD HH:mm:ss");
      let logdatetime = moment(new Date(req.body.logdatetime)).format("YYYY-MM-DD HH:mm:ss");
      let lastmodifieddatetime = moment(new Date(req.body.lastmodifieddatetime)).format("YYYY-MM-DD HH:mm:ss");
...
router.post('/storage/save/whatsapp',
  function (req, res) {
    let storage_logid_uuid = uuidv4();
